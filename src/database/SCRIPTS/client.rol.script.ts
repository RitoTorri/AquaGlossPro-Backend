...
async function clientRoleSeed() {
  ...
  try {
    ...
    // 1. Crear o obtener el Rol CLIENT
    console.log('--- Creando el rol CLIENT ---');
    let clientRole = await queryRunner.manager.findOne(Role, {
      where: { name: 'CLIENT' },
    });

    if (!clientRole) {
      clientRole = queryRunner.manager.create(Role, {
        name: 'CLIENT',
        active: true,
      });
      clientRole = await queryRunner.manager.save(clientRole);
      console.log('✅ Rol CLIENT creado correctamente.');
    } else {
      console.log('ℹ️  Rol CLIENT ya existe.');
    }

    // 2. Definir los permisos por módulo (MODIFICAR AQUÍ PARA AGREGAR MÁS PERMISOS)
    ...
    console.log('--- Asignando permisos al rol CLIENT ---');
